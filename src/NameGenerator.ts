// Borrowed from https://github.com/moby/moby/ -> names-generator.go ,
// with some additions
...
const people = [
  ...
  // Helga Dietrich - German orchidologist and author who is noted for her comprehensive guides to literature on orchids
  'dietrich',
  ...
  // Alan L. Hart - American physician, radiologist, tuberculosis researcher, writer, and novelist
  'hart',
  ...
  // Mary Ann Horton = is a Usenet and Internet pioneer. Horton contributed to Berkeley UNIX (BSD)
  'horton',
  ...
  // Rachael Padman - an Australian physics lecturer at the University of Cambridge in England
  'padman',
...
const prefixes = [ ...adjectives, ...colours ];
const suffixes = [ ...people, ...creatures ];

const namesByUserId: Record<string, string> = {};
const generatedNames: Set<string> = new Set<string>();

/**
 * Generate a name based on a Discord user ID. We should
 * be looking to generate the same name no matter the number
 * of times we execute, but for now its just a best effort to
 * test the concept.
 *
 * @param userId
 * @returns
...
function generateName (userId: string): string {
  if (namesByUserId[userId]) {
    return namesByUserId[userId];
  }

  const left = parseInt(userId.substring(0, userId.length / 2));
  const right = parseInt(userId.substring((userId.length / 2), userId.length));

  let name = `${prefixes[left % prefixes.length]}-${suffixes[right % suffixes.length]}`;

  if (generatedNames.has(name)) {
    let idx = 42;
    let tmpName = `${name}${idx}`;
    while (generatedNames.has(tmpName)) {
      tmpName = `${name}${idx}`;
    }
    name = tmpName;
  }
